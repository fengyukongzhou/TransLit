...
- 默认使用简体中文
- 【脚注保护】原文中的脚注引用 [^N] 和脚注定义 [^N]: 必须原样保留。**特别注意**：脚注定义中的冒号必须是半角冒号加空格（即 [^N]: ），绝对不可翻译为全角冒号（：）。不得修改编号、删除、重新排序或翻译标记本身。只翻译脚注定义中的文字内容。
`;
...
3. **一致性**：参考当前表格，确保全文核心译名与特殊词意绝对统一。提取的译文目标必须与当前正文实际翻译结果**完全一致**。`;
...
<glossary>
只提取本段新发现且未在已知术语表中出现过的高风险/专有词汇。严禁发散。若无新术语则留空该标签内心：
...
3. **严禁篡改译文（极其重要）**：对于保留下来的词条，**绝对不可以改变其原有的【译文】**。你的任务仅限于删除和去重，绝不能创造或更改任何已有内容。
